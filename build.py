...
<style>
  *,*::before,*::after{box-sizing:border-box}
  ...
  /* A bordered pill flush with the table's left edge, with more room above it than
     below, so it reads as attached to what follows rather than to the key above. */
  #found{margin:1.7rem 0 -.55rem}
  #found .reset{margin-left:0;text-decoration:none;border:1px solid var(--rule);
                border-radius:999px;padding:.28rem .72rem;color:var(--soft);
                font:400 12.5px/1 ui-sans-serif,system-ui,sans-serif}
  #found .reset:hover{border-color:var(--accent);color:var(--accent)}
  ...
  /* The two headers carrying a unit put it on a second line as a SUB-LABEL rather
     than by wrapping. Wrapping recovered the width an eleventh column needed, but it
     broke mid-phrase and left the sort arrow stranded on line two, which read as a
     mistake. As a styled unit it reads as a two-line header, which is an ordinary
     table convention, and it is narrower still. */
  th .u{display:block;font-weight:400;font-size:9.5px;letter-spacing:.06em;
        opacity:.62;margin-top:.1rem}
...
<div class="wrap">
  ...
  <!-- Directly above the table, because it clears the filter the table is showing the
       effect of. It sat above the pager for one round and read as belonging to the key
       further up. A bare button by design: see tests/test_calc.py for why it carries no
       caption. -->
  <p id="found" hidden><button class="reset" id="showall" type="button">Clear</button></p>
  <div class="tablewrap">
  <table>
    <thead><tr>
      ...
      <th data-k="gap" class="n">Gap<span class="ar">&#9650;</span><span class="u">(pp)</span></th>
      <th data-k="mcap" class="n mc-h">Cap<span class="ar">&#9650;</span><span class="u">(US$bn)</span></th>
    ...
    <tr class="filters">
      ...
      <td class="n mc"><input id="c-mcap" type="text" inputmode="decimal" placeholder="min" aria-label="Minimum market cap in US$ billions"></td>
...
(function () {
  ...
  function draw(rows, marked) {
    ...
    order_.forEach(function (r) {
      ...
      // Its OWN radius, not the shared default: mouseleave used to reset every dot to
      // 5.2, which quietly shrank a ringed company the moment the pointer passed over
      // it and left it looking like any other point.
      c.__r = hit ? 8.4 : 5.2;
      svg.appendChild(c);
      drawn.push(c);
    });
    ...
    svg.addEventListener("mousemove", function (ev) {
      ...
      if (current) current.setAttribute("r", current.__r);
      current = best;
      if (!best) { tip.style.opacity = 0; return; }
      best.setAttribute("r", Math.max(8, best.__r + 1.6));
    ...
    /* Clicking a point names that company: the table filters to it and the point keeps
       its ring, which is the same state a link from Shortfall or DCF Studio arrives in.
       The dots have carried cursor:pointer since the chart was built and nothing
       happened when you clicked one. Resolved to the nearest point, like the tooltip -
       an exact hit on a 5px circle is not a fair thing to ask of a pointer. */
    svg.addEventListener("click", function (ev) {
      ...
      if (!best) return;
      els.text.value = best.__row.ticker;
      landed = true;              // the chart keeps the population and rings the pick
      page = 0;
      tip.style.opacity = 0;
      current = null;
      writeQuery();
  ...
  function apply() {
    ...
    /* Offered whenever the NAME box is narrowing the table, whether the reader typed
       it or arrived with it. The Clear filters link at the top of the page resets
       everything; this one undoes the single filter the reader is looking at the
       effect of, from where they are looking at it. */
